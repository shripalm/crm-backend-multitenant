...
# assign_task_for_contact_presales
# assign_task_for_call_report_sales
# assign_task_for_lead_site_visit
from app.services.auto_assign_service import (
    assign_task_for_contact_presales,
    assign_task_for_call_report_sales,
    assign_task_for_lead_site_visit,
)
...
async def interested_func(db: AsyncSession, action: dict, status: str = "interested") -> bool:
    ...
    try:
        ...
        logger.debug(
            f"{status} workflow contact lookup",
            task_id=str(task_id),
        ...
        if current_lead_id is not None:
            ...
            if lead_data is not None:
                ...
                next_autoAssign = crm_data_leads_wf_definition.get(next_stage, {}).get("auto_assign_func")
                ...
                activity = next_table(
                    ...
                    # from data
                    remark=f"Lead marked as {status} - {remarks}" if remarks else f"Lead marked as {status}",
                    **(_data or {})
                )
                db.add(activity)
                await next_autoAssign(db, activity)
                logger.info(f"Logged {status} activity", contact_id=str(activity.id), task_id=str(task_id))

        await db.commit()
        await db.refresh(task)
        logger.info("Task marked as interested", task_id=str(task_id))
        return success_response(data={"id": task_id}, message=f"Marked as {status}")
...
async def lead_drop_func(db: AsyncSession, action: object) -> bool:  # not interested
    try:
        ...
        # if assigned_to_team = presales, then add if condition
        task.remarks = action["data"]["remarks"] or task.remarks
        await db.commit()
        await db.refresh(task)
        logger.info("Task marked as not interested", task_id=str(task_id))
        if task.assigned_to_team == "presales":
            return await interested_func(db, action, status="not interested")
        else:
            return success_response(data={"id": task_id}, message="Marked as Not Interested")
    except Exception as e:
        logger.error(
            f"Failed not interested workflow for task {task_id}: {str(e)}")
        return internal_server_error(f"Failed to update task: {str(e)}")
...
crm_data_leads_wf_definition = {
    "raw": {
        ...
        "next_stage": "presales",
        "prev_stage": None,
        "auto_assign_func": assign_task_for_contact_presales,
    },
    "presales": {
        "table": CallReport,
        "next_stage": "sales",
        "prev_stage": "raw",
        "auto_assign_func": assign_task_for_call_report_sales,
    },
    "sales": {
        "table": Lead,
        "next_stage": "sitevisit",
        "prev_stage": "presales",
        "auto_assign_func": assign_task_for_lead_site_visit,
    },
    "sitevisit": {
        "table": SiteVisit,
        "next_stage": None,
        "prev_stage": "sales",
        "auto_assign_func": None,
    },
}
